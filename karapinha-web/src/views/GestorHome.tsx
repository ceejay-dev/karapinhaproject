...
export function GestorHome() {
  ...
  const [totalAmountYesterday, setTotalAmountYesterday] = useState<
    number | null
  >(null);
  const [totalAmountCurrentMonth, setTotalAmountCurrentMonth] = useState<
    number | null
  >(null);
  const [totalAmountPastMonth, setTotalAmountPastMonth] = useState<
    number | null
  >(null);
  ...
  useEffect(() => {
    ...
    async function fetchMonthlyBookings() {
      try {
        const response = await fetch(
          "https://localhost:7209/GetBookingsByMonth"
        );
        if (response.ok) {
          const data = await response.json();
          ...
          const updatedBookings = await Promise.all(
            data.map(async (marcacao: Marcacao) => {
              const updatedServicos = await Promise.all(
                marcacao.servicos.map(async (servico) => {
                  try {
                    const serviceResponse = await fetch(
                      `https://localhost:7209/GetTreatmentById?id=${servico.fkServico}`
                    );
                    ...
                    const professionalResponse = await fetch(
                      `https://localhost:7209/GetProfissionalById?id=${servico.fkProfissional}`
                    );
                    if (professionalResponse.ok) {
                      const professionalData =
                        await professionalResponse.json();
                      servico.nomeProfissional =
                        professionalData.nomeProfissional;
                    } else {
                      console.error(
                        "Erro ao buscar o profissional:",
                        professionalResponse.statusText
                      );
                    }

                    const scheduleResponse = await fetch(
                      `https://localhost:7209/GetSchedule?id=${servico.fkHorario}`
                    );
          ...
          console.error(
            "Erro ao buscar as marcações mensais:",
            response.statusText
          );
        }
      } catch (error) {
    ...
    async function fetchTotalAmountToday() {
      try {
        const response = await fetch(
          "https://localhost:7209/GetTotalAmountToday"
        );
        if (response.ok) {
          const data = await response.json();
          ...
          console.error(
            "Erro ao buscar o total faturado hoje:",
            response.statusText
          );
        }
      } catch (error) {
    ...
    async function fetchTotalAmountYesterday() {
      try {
        const response = await fetch(
          "https://localhost:7209/GetTotalAmountYesterday"
        );
        if (response.ok) {
          const data = await response.json();
          ...
          console.error(
            "Erro ao buscar o total faturado ontem:",
            response.statusText
          );
        }
      } catch (error) {
    ...
    async function fetchTotalAmountCurrentMonth() {
      try {
        const response = await fetch(
          "https://localhost:7209/GetTotalAmountCurrentMonth"
        );
        if (response.ok) {
          const data = await response.json();
          ...
          console.error(
            "Erro ao buscar o total faturado no mês atual:",
            response.statusText
          );
        }
      } catch (error) {
    ...
    async function fetchTotalAmountPastMonth() {
      try {
        const response = await fetch(
          "https://localhost:7209/GetTotalAmountPastMonth"
        );
        if (response.ok) {
          const data = await response.json();
          ...
          console.error(
            "Erro ao buscar o total faturado no mês passado:",
            response.statusText
          );
        }
      } catch (error) {
  ...
  return (
    <section className="principal-container">
      ...
      <div className="dashboard">
        <h4 className="text-white text-center m-0 bg-dark desc">
          Agenda Mensal
        </h4>
        ...
        <div className="custom-card-container">
          ...
          <Card className="custom-card bg-dark text-white">
            ...
            <Card.Body className="custom-card-body">
              ...
              <Card.Text>
                Ontem:{" "}
                {totalAmountYesterday !== null
                  ? `${totalAmountYesterday} kz`
                  : "Carregando..."}
              </Card.Text>
              <Card.Text>
                Mês actual:{" "}
                {totalAmountCurrentMonth !== null
                  ? `${totalAmountCurrentMonth} kz`
                  : "Carregando..."}
              </Card.Text>
              <Card.Text>
                Mês Passado:{" "}
                {totalAmountPastMonth !== null
                  ? `${totalAmountPastMonth} kz`
                  : "Carregando..."}
              </Card.Text>
            </Card.Body>
          </Card>
